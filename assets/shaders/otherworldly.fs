...
extern MY_HIGHP_OR_MEDIUMP vec2 otherworldly;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float t = otherworldly.y*2.221 + time;
    ...
    float cx = uv_scaled_centered.x * 2;
    ...
    float wave1 = pow(pow(mod2(field*5 + cy1 + cx1*cx1*0.5 + msin(cx1 - cy1 + t/3) - t/11, 4) - 2, 2) + 1, -1);
    
    cx1 *= -0.71;
    cy1 *= -2.3;

    wave1 = 2 + wave1 + pow(pow(mod2(field*5 + cy1 + cx1*cx1*0.5 + msin(cx1 - cy1 + t/3) - t/11, 4) - 2, 2) + 1, -1);

    wave1 /= 5;
    tex.r = max(0, tex.r - wave1 * 0.7);
    tex.g = max(0, tex.g - wave1 * 0.5);
    ...
    for(int i = 0; i < 5; i++)
    {
        sw1 += swirl(cx/8 + cos(i*2.4) * 3, cy/8 + sin(i*2.4) * 3, 1, t/8);
        for(int j = 0; j < 7; j++)
        {
            st1 += star(
                cx/8 + sin(i + j * 2.4 + t/(12+i*3)) * (3 + i + sin(j)/3),
                cy/8 + cos(i + j * 2.4 + t/(12+i*3)) * (3 + i + cos(j)/3),
                0.9 + sin(j/7 + i/5)/8,
                0.4
                )
                * (0.8 + abs(msin(t/6 + 261.13*i - 83.66*j)))
                ;
        }
    }
    hsl.z -= abs(sw1/3);

    hsl.x = hsl.x*0.3 + 0.85*0.7 + field/12;

    tex.rgb = RGB(hsl).rgb;

    tex.rgb = tex.rgb - (pow(cx/40, 2) + pow(cy/40, 2) + msin(sw1)/7)/4;
	...
	return dissolve_mask(pixel * 0.4 + tex*colour * 0.6, texture_coords, uv);
}
